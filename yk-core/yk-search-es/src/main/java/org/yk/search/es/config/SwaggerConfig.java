package org.yk.search.es.config;
...
public class SwaggerConfig {
	...
	public Docket swaggerSpringMvcPlugin() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).genericModelSubstitutes(DeferredResult.class)
				...
				.apis(RequestHandlerSelectors.basePackage("org.yk.search.es.controller")).paths(PathSelectors.any())
				.build();
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("search es").description("yk-cloud Platform  API")
